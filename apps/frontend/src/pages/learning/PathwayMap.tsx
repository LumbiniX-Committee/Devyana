  ...
  return (
    ...
    >
      <div className="mb-6 flex flex-wrap items-start justify-between gap-3">
        <div>
          <p className="text-xs font-medium tracking-wide" style={{ color: MUTED, fontFamily: FONT }}>
            Structured study
          </p>
          <h2 className="mt-1 buddha-heading text-xl" style={{ color: INK }}>
            {pathway.title}
          </h2>
          <p className="mt-1 text-sm" style={{ color: MUTED, fontFamily: FONT }}>
            {pathway.description}
          </p>
        </div>
        <div className="flex flex-wrap items-center gap-4" aria-label="Lesson status legend">
          <span className="flex items-center gap-1.5 text-xs" style={{ color: MUTED, fontFamily: FONT }}>
            <i className="legend-dot completed h-2 w-2 rounded-full" style={{ backgroundColor: SAGE }} />
            Completed
          </span>
          <span className="flex items-center gap-1.5 text-xs" style={{ color: MUTED, fontFamily: FONT }}>
...
`).trim()}")`,
        ...
        <Flower2 className="absolute top-2 left-2 h-8 w-8" aria-hidden="true" style={{ color: SAGE, opacity: 0.6 }} />
        <CircleDot className="absolute bottom-2 right-2 h-8 w-8" aria-hidden="true" style={{ color: TERRACOTTA, opacity: 0.6 }} />

        <span className="absolute path-particle particle-one h-1.5 w-1.5 rounded-full" style={{ backgroundColor: SAGE, opacity: 0.4, top: "15%", left: "20%" }} aria-hidden="true" />
        <span className="absolute path-particle particle-two h-1.5 w-1.5 rounded-full" style={{ backgroundColor: TERRACOTTA, opacity: 0.4, top: "50%", left: "60%" }} aria-hidden="true" />
        <span className="absolute path-particle particle-three h-1.5 w-1.5 rounded-full" style={{ backgroundColor: "#D4A853", opacity: 0.4, top: "75%", left: "25%" }} aria-hidden="true" />
        ...
        {pathway.nodes.map((node, index) => {
          ...
          return (
            ...
            >
              ...
              >
                {node.status === "available" && <span className="node-pulse absolute inset-0 rounded-full animate-pulse" style={{ backgroundColor: TERRACOTTA, opacity: 0.3 }} aria-hidden="true" />}
                <span className="relative flex items-center justify-center flex-col z-10">
                  <span className="node-index text-[9px] font-medium" style={{ color: INK, fontFamily: FONT }} aria-hidden="true">
                    {String(index + 1).padStart(2, "0")}
                  </span>
                  <span className="node-icon mt-1"><NodeIcon node={node} /></span>
                </span>
              </button>
              ...
              >
                ...
                <span className="block text-[#D4A853] mb-1">{node.description}</span>
                <em className="block text-[#8B9A6E]">{statusLabel[node.status]}</em>
              </div>
            </div>
          );
        })}
