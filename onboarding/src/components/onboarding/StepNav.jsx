 ...
 */
export const StepNav = ({ total, index, onBack }) => {
  return (
    <motion.div
    ...
    >
      ...
      <div className="flex items-center gap-2" role="progressbar" aria-valuenow={index + 1} aria-valuemax={total}>
        {Array.from({ length: total }).map((_, i) => (
          <motion.span
            ...
            className={cn(
              "h-2 rounded-full",
              i === index ? "bg-primary" : i < index ? "bg-foreground/60" : "bg-foreground/25"
            )}
          />
        ))}
      </div>
